...
public class TrainSvBPFF extends TrainSv
{
     ...
     *
     * @param transFnct the trans fnct
	...
	// BPError= w' * error
	DoubleMatrix error =
	    destLayer.getWeight().transpose().mmul(destLayer.getError());
     ...
     *
     * @param diffTransfereFnctr the new diff transfere fnctr
     */
	...
	{
	    double performance =
		stochasticOnce(net, inputList.get(i), targetList.get(i));
     ...
     * @return the train result
     */
    public TrainResult stochasticToLimits(FeedForwardSvNet net, DoubleMatrix input,
	DoubleMatrix target)
    {
	TrainResult result = new TrainResult();
	result.start();
	...
	// update
	DoubleMatrix dw =
	    error.mul(lrnRate).mul(diffOutput).mmul(inputTransposed);
