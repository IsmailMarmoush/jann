...
public class NNFeedForwardBPNet extends ArrayList<SvLayer>
{
     ...
     */
    public void
	trainOnce(final DoubleMatrix newInput, final DoubleMatrix target)
    {
	...
	for (SvLayer lyr : this)
	{
	    ...
	    // lyr.getNetsum(), 0);
	    deltaWeight =
		deltaRuleBp(lyr.getInput().transpose(), lyr.getError(),
		    diffOutput, lyr.getLearnRate());
	...
	for (int i = lastLayer; i > 0; i--)
	{
	    lyr = get(i);
	    preLyr = get(i - 1);
	    preLyr.setError(backPropagate(lyr.getWeight(),
		lyr.getError()));
	}
    }
}